...
router.post("/register", async (req, res) => {
  try {
    ...
    if (!userId) return res.status(401).json({ code: "unauthenticated", message: "Authentication required" });
    ...
    return res.status(500).json({ code: "server_error", message: "Failed to save push token" });
  }
});
